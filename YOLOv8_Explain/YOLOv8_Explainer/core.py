...
from ultralytics.nn.tasks import attempt_load_weights, DetectionModel
...
class ActivationsAndGradients:
    ...
    def save_activation(self, module: torch.nn.Module,
        ...
        self.activations.append(activation.cpu().detach().float())  # Convert to float32
    ...
    def save_gradient(self, module: torch.nn.Module,
        ...
        def _store_grad(grad: torch.Tensor) -> None:
            ...
            self.gradients = [grad.cpu().detach().float()] + self.gradients  # Convert to float32
    ...
    def __call__(self, x: torch.Tensor) -> List[List[Union[torch.Tensor, np.ndarray]]]:
        ...
        self.gradients = []
        self.activations = []
        # Convert input to model's dtype for forward pass
        model_dtype = next(self.model.parameters()).dtype
        x_model = x.to(dtype=model_dtype)
        model_output = self.model(x_model)
...
class yolov8_heatmap:
    ...
    def __init__(
        ...
        # This is safe since we're loading a trusted model file
        ckpt = torch.load(weight, weights_only=False)
        model_names = ckpt['model'].names

        # Load model directly from checkpoint to avoid ultralytics torch_safe_load issues
    ...
    def process(self, img_path):
        ...
        # Convert to model's dtype for forward pass, but keep float32 for CAM computation
        model_tensor = tensor.to(dtype=next(self.model.parameters()).dtype)

        try:
            grayscale_cam = self.method(tensor, [self.target])  # Use float32 tensor for CAM
        ...
        pred1 = self.model(model_tensor)[0]  # Use model_dtype tensor for model forward pass
